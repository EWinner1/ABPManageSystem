...
using Ewinner.ManageSystem.Blazor.WebApp.Tiered.Client;
using Ewinner.ManageSystem.Blazor.WebApp.Tiered.Client.Menus;
using Ewinner.ManageSystem.Blazor.WebApp.Tiered.Components;
using Ewinner.ManageSystem.Blazor.WebApp.Tiered.Menus;
...
namespace Ewinner.ManageSystem.Blazor.WebApp.Tiered;

[DependsOn(
	typeof(ManageSystemHttpApiClientModule),
...
public class ManageSystemBlazorModule : AbpModule
{
	...
	private void ConfigureCache()
	{
		...
		{
			options.KeyPrefix = "ManageSystem:";
		});
	}
		...
		Configure<AbpBundlingOptions>(options =>
		{
			...
			options.StyleBundles.Configure(
				...
				bundle =>
				{
					...
					bundle.AddFiles(new BundleFile("/Ewinner.ManageSystem.Blazor.WebApp.Tiered.Client.styles.css", true));
				}
			);
		});
	}
	...
	private void ConfigureAuthentication(ServiceConfigurationContext context, IConfiguration configuration)
	{
		context.Services.AddAuthentication(options =>
			...
			.AddAbpOpenIdConnect("oidc", options =>
			{
				...
				options.Scope.Add("ManageSystem");
			});
		/*
	...
	private void ConfigureMenu(IConfiguration configuration)
	{
		...
		{
			options.MenuContributors.Add(new ManageSystemMenuContributor(configuration));
		});

		Configure<AbpToolbarOptions>(options =>
		{
			options.Contributors.Add(new ManageSystemToolbarContributor());
		});
	}
	...
	{
		var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("ManageSystem");
		if (!hostingEnvironment.IsDevelopment())
		{
			var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]!);
			dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "ManageSystem-Protection-Keys");
		}
	}
